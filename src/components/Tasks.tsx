...
const Tasks: FC<TasksProps> = ({ tasks }) => {
	...
	const setTaskToEditHandlet = (task: Task) => {
		dispatch(setTaskToEdit(task, list))
	}

	const setTaskToDeleteHandlet = (task: Task) => {
					...
					tasks.map((task: Task) => (
						<tr key={task.id} className={task.completed ? 'completed' : ''}>
							...
							<td className="has-text-centered">
								<button onClick={() => setTaskToEditHandlet(task)} className="button is-primary is-small">
									<span className="icon">
							...
							<td className="has-text-centered">
								<button onClick={() => setTaskToDeleteHandlet(task)} className="button is-danger is-small">
									<span className="icon">
